...
'config.json')
if (configFilePath.substring(configFilePath.length - 5, configFilePath.length) === '.yaml' || configFilePath.substring(configFilePath.length - 4, configFilePath.length) === '.yml') {
  fileConfig = fs.existsSync(configFilePath) ? yaml.safeLoad(fs.readFileSync(configFilePath, 'utf8'))[env] : undefined
} else {
  fileConfig = fs.existsSync(configFilePath) ? require(configFilePath)[env] : undefined
}
